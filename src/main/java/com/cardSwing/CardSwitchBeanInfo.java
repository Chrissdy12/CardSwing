...
public class CardSwitchBeanInfo extends SimpleBeanInfo {

    @Override
    public BeanDescriptor getBeanDescriptor() {
        BeanDescriptor bd = new BeanDescriptor(CardSwitch.class);
        bd.setDisplayName("CardSwitch");
        return bd;
    }
    
    @Override
    public java.awt.Image getIcon(int iconKind) {
        return CardSwingIcons.getIconFor("CardSwitch");
    }
    ...
    public PropertyDescriptor[] getPropertyDescriptors() {
        try {
            ...
            for (String propName : customProps) {
                try {
                    PropertyDescriptor pd = new PropertyDescriptor(propName, CardSwitch.class);
                    pd.setPreferred(true);
                    pd.setValue("category", "CardSwitch Configs");
                    props.add(pd);

...
public class CardStatusRowBeanInfo extends SimpleBeanInfo {

    @Override
    public BeanDescriptor getBeanDescriptor() {
        BeanDescriptor bd = new BeanDescriptor(CardStatusRow.class);
        bd.setDisplayName("CardStatusRow");
        return bd;
    }
    
    @Override
    public java.awt.Image getIcon(int iconKind) {
        return CardSwingIcons.getIconFor("CardStatusRow");
    }
    ...
    public PropertyDescriptor[] getPropertyDescriptors() {
        try {
            ...
            for (String propName : customProps) {
                try {
                    PropertyDescriptor pd = new PropertyDescriptor(propName, CardStatusRow.class);
                    pd.setPreferred(true);
                    pd.setValue("category", "CardStatusRow Configs");
                    props.add(pd);

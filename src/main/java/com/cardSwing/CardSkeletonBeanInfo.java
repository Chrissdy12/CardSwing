...
public class CardSkeletonBeanInfo extends SimpleBeanInfo {

    @Override
    public BeanDescriptor getBeanDescriptor() {
        BeanDescriptor bd = new BeanDescriptor(CardSkeleton.class);
        bd.setDisplayName("CardSkeleton");
        return bd;
    }
    
    @Override
    public java.awt.Image getIcon(int iconKind) {
        return CardSwingIcons.getIconFor("CardSkeleton");
    }
    ...
    public PropertyDescriptor[] getPropertyDescriptors() {
        try {
            ...
            for (String propName : customProps) {
                try {
                    PropertyDescriptor pd = new PropertyDescriptor(propName, CardSkeleton.class);
                    pd.setPreferred(true);
                    pd.setValue("category", "CardSkeleton Configs");
                    props.add(pd);

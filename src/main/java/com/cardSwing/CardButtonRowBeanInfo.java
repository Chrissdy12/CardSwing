...
public class CardButtonRowBeanInfo extends SimpleBeanInfo {

    @Override
    public BeanDescriptor getBeanDescriptor() {
        BeanDescriptor bd = new BeanDescriptor(CardButtonRow.class);
        bd.setDisplayName("CardButtonRow");
        return bd;
    }
    
    @Override
    public java.awt.Image getIcon(int iconKind) {
        return CardSwingIcons.getIconFor("CardButtonRow");
    }
    ...
    public PropertyDescriptor[] getPropertyDescriptors() {
        try {
            ...
            for (String propName : customProps) {
                try {
                    PropertyDescriptor pd = new PropertyDescriptor(propName, CardButtonRow.class);
                    pd.setPreferred(true);
                    pd.setValue("category", "CardButtonRow Configs");
                    props.add(pd);

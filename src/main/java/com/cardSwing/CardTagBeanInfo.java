...
public class CardTagBeanInfo extends SimpleBeanInfo {

    @Override
    public BeanDescriptor getBeanDescriptor() {
        BeanDescriptor bd = new BeanDescriptor(CardTag.class);
        bd.setDisplayName("CardTag");
        return bd;
    }
    
    @Override
    public java.awt.Image getIcon(int iconKind) {
        return CardSwingIcons.getIconFor("CardTag");
    }
    ...
    public PropertyDescriptor[] getPropertyDescriptors() {
        try {
            ...
            for (String propName : customProps) {
                try {
                    PropertyDescriptor pd = new PropertyDescriptor(propName, CardTag.class);
                    pd.setPreferred(true);
                    pd.setValue("category", "CardTag Configs");
                    props.add(pd);
